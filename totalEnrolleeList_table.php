...
$sql = "SELECT * FROM enrollee WHERE status = 'PENDING'";
...
if (null !== $result && $result->num_rows > 0) {
    ...
    echo '<th scope="col" colspan="9" style="text-align:center;">Enrollee List</th>';
    echo '</tr>';
    echo '</thead>';
    echo '<thead class="thead-dark">';
    echo '<tr>';
   ...
   //  echo '<th scope="col">Actions</th>'; // add new column for buttons
    echo '</tr>';
    echo '</thead>';
    echo '<tbody>';
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo '<td>';
        echo '<button type="button" class="btn btn-success" data-enrollee-id="' . $row["enrolleeId"] . '" id="enrolleeDetailsButton" style="margin-right:10px;" onclick="showEnrolleeDetailsModal(' . $row["enrolleeId"] . ')">View Details</button>';


        echo '<button type="button" class="btn btn-success" data-enrollee-id="' . $row["enrolleeId"] . '" onclick="approveEnrollmentRequest(' . $row["enrolleeId"] . ')">Approve</button>';
        echo '</td>';
        echo '</tr>';
    }
    ...
    echo '<h5 class="modal-title" id="enrolleeDetailsModalLabel">Enrollee Details</h5>';
    echo '<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label=s"Close"></button>';
...
<script>
    ...
    function showEnrolleeDetailsModal(enrolleeId) {
        var id = enrolleeId;
        var buttonId = "enrolleeDetailsButton"
        var button = document.getElementById(buttonId);
        $.ajax({
            url: "get_enrollee_details.php",
            type: "POST",
            data: {
                enrolleeId: id
            },
...
<script>
    function approveEnrollmentRequest(enrolleeId) {
        var id = enrolleeId;
        $.ajax({
            url: "approve_enrollee.php",
            type: "POST",
            data: {
                enrolleeId: id
            },
